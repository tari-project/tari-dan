...
async fn handler(
    ...
    match value.method.as_str().split_once('.') {
        ...
        Some(("webrtc", method)) => match method {
            ...
            "accept_request" => webrtc::handle_accept_request(context, value, token, message_queue, addresses.0).await,
            "deny_request" => webrtc::handle_deny_request(context, value, token, message_queue).await,
            _ => Ok(value.method_not_found(&value.method)),
        },
