    ...
    let result = template_test
        .execute_and_commit(
            vec![Instruction::CallFunction {
                ...
                function: "test_function_without_component_address".to_string(),
                args: args![],
            }],
            vec![],
        )
...
#[test]
fn basic_emit_event_with_component_address() {
    let mut template_test = TemplateTest::new(vec!["tests/templates/events"]);
    let event_emitter_template = template_test.get_template_address("EventEmitter");
    let result = template_test
        .execute_and_commit(
    ...
    assert_eq!(result.finalize.events[0].template_address(), event_emitter_template);
    assert_eq!(
        result.finalize.events[0].component_address(),
        Some(ComponentAddress::from_array([0u8; 32]))
    );
    assert_eq!(
        result.finalize.events[0].get_payload("my").unwrap(),
        "event".to_string()
